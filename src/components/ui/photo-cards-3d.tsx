...
function PhotoCard({ src, alt, rotation, caption, index, style }: PhotoCardProps) {
  ...
  const cardStyle: CSSProperties = {
    position: 'absolute',
    transform: `rotate(${rotation}deg) ${isHovered ? `rotate(${rotation + 2}deg) scale(1.04)` : `rotate(${rotation}deg) scale(1)`}`,
  ...
  return (
    <div
      className={cn(
        'w-[min(88vw,320px)] cursor-pointer rounded-md border border-chocolate-800/25 bg-marshmallow p-2.5 shadow-2xl sm:w-[min(80vw,380px)] sm:p-3 md:w-[min(72vw,420px)] md:p-3.5',
        isHovered && 'shadow-[0_25px_50px_-12px_rgba(0,0,0,0.4)]',
      )}
      style={cardStyle}
